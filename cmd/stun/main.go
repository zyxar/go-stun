...
func main() {
	...
	if len(ips) > 1 {
		...
		for {
			...
			if nil != err {
				fmt.Println(fmt.Sprintf("The given value (%s) is invalid.", response))
				continue
			}

			if (idx < 0) || (idx >= len(ips)) {
				fmt.Println(fmt.Sprintf("The given value (%d) is invalid.", idx))
				continue
			}
	...
	if nat, err = stun.ClientDiscover(); nil != err {
